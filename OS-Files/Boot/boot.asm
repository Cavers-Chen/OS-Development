; this is a sector booting program

[org 0x7c00]

mov [BOOT_DRIVE], dl

mov bx, BOOT_Name
call print
call println

mov dx, 0xAA55
call print_hex
call println

; there is no diffrent with the first one, 
; because they are all restored in capitcal alphabeta
mov dx, 0xaa55 
call print_hex
call println

mov bx, BOOT_NOTE
call print
call println

; read disk

mov bp, 0x8000 ; set the stack safely away from us
mov sp, bp

mov bx, 0x9000 ; es:bx = 0x0000:0x9000 = 0x09000
mov dh, 2 ; read 2 sectors
mov dl, [BOOT_DRIVE]
; the bios sets 'dl' for our boot disk number
; if you have trouble, use the '-fda' flag: 'qemu -fda file.bin'
call disk_load

mov dx, [0x9000] ; retrieve the first loaded word, 0xdada
call print_hex
call println

mov dx, [0x9000 + 512] ; first word from second loaded sector, 0xface
call print_hex
call println

jmp $

%include "print_lib.asm"
%include "../Disk/disk_load.asm"

BOOT_Name:
db 'Cavers Operation System! This Program end with', 0

BOOT_NOTE:
db 'Booting...',0

BOOT_DRIVE : 
db 0

times 510-($-$$) db 0
dw 0xaa55

; boot sector = sector 1 of cyl 0 of head 0 of hdd 0
; from now on = sector 2 ...
times 256 dw 0xdada ; sector 2 = 512 bytes
times 256 dw 0xface ; sector 3 = 512 bytes

; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; |                                       |
; |                                       |
; |                                       |
; |               FREE                    |
; |                                       |
; |                                       |
; |_______________________________________|<--0x100000
; |            BIOS(256KB)                |
; |_______________________________________|<--0xC0000
; |         Video Memory(128KB)           |
; |_______________________________________|<--0xA0000
; |      Extended BIOS Data Area(638KB)   |
; |_______________________________________|<--0x9FC00
; |              Free(638KB)              |
; |_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _|<--0x7E00
; |      Loaded Boot Sector(512 Bytes)    |
; |_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _|<--0x7C00
; |                                       |
; |_______________________________________|<--0x500
; |         BIOS Data Area(256Bytes)      |
; |_______________________________________|<--0x400
; |      Interrupt Vector Table(1KB)      |
; |_______________________________________|<--0x0